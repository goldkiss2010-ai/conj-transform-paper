...
一様乱数サンプルおよび実写画像に対する数値実験では，従来型パイプライン（例：ガンマ補正後の Y′CbCr 操作，CIELAB 明度操作）において，
...
ガンマ補正関数，トーンマッピング，ロールオフなどを含む任意の非線形を想定する \citep{reinhard2005hdr,itur_bt2100}．
...
YCbCr は $R'G'B'$（ガンマ補正後 RGB）を作業空間として，
線形結合により輝度相当の量と色差を構成する \citep{itur_bt601,itur_bt709}．
見かけ上は $1+2$ 形式であるが，分割の前にガンマ補正が三成分全体へ作用しているため，
...
\paragraph{読者ガイド（本章の目的）}
本章の比較は，厳密な規格準拠パイプラインの再現ではなく，
「非線形をどの段に置くか（非線形$\to$分解／分解$\to$非線形）」という順序の差が，
scene-linear 層に引き戻した残差にどの程度クロストークとして現れるかを可視化するための
\textbf{簡約モデル}である．
sRGB/XYZ/BT.709 などの表記は「現場で典型的に現れる定義」を代表させるために用いる．

\subsection*{記号一覧（Notation）}
\begin{table}[t]
\centering
\small
\begin{tabular}{ll}
...
記号 & 意味 \\
\midrule
$V$ & 色ベクトル空間（例：$\mathbb{R}^3$）\\
$x$ & サンプル（画素）ベクトル（RGB/XYZなど）\\
$u$ & 主軸ベクトル（無彩色軸・輝度軸など）\\
$\ell$ & 読み取り汎関数（主軸スカラー；例：$\ell(x)=c^\top x$）\\
$\ell(u)=1$ & 双対正規化（主軸と読み取りの整合条件）\\
$P(x)$ & 残差射影：$P(x)=x-\ell(x)u\in\ker\ell$\\
$r(x)$ & 残差：$r(x)=P(x)$\\
$\phi$ & 1Dトーン関数（例：$\phi(s)=\max(s,0)^\alpha$）\\
$\alpha$ & トーン関数の指数パラメータ\\
$\gamma$ & ガンマ補正（Legacy側の非線形；簡約モデル）\\
$F$ & パイプライン全体を表す写像（Legacy/CONJ）\\
$D$ & 評価サンプル集合（乱数サンプル or 画素集合）\\
$C(F;D)$ & クロストーク指標（式\eqref{eq:crosstalk}）\\
$\Sigma,\ \Delta\Sigma$ & 残差共分散とその差（式\eqref{eq:deltasigma}）\\
$\|\cdot\|_2,\ \|\cdot\|_F$ & $\ell_2$ノルム，Frobeniusノルム\\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{注記（Legacy の簡約）}
本稿の $R'G'B'\leftrightarrow Y'CbCr$ は，順序不整合を代表させるための
\textbf{簡易モデル}であり，規格におけるレンジ（limited/full）やオフセット，
量子化を含む厳密な伝送実装の再現を目的としない．
使用した具体式は付録Bを参照．
...
\noindent
\textit{（注）} $E^{-1}$ は sRGB 逆EOTF（成分ごと）であり，具体式は付録Bに示す．
...
\section{実験で用いた簡易変換の定義（補足）}
\subsection{簡易 $R'G'B' \leftrightarrow Y'CbCr$（フルレンジ）}
本稿の Legacy では，順序不整合を代表させるため，
BT.709 係数 $(K_r,K_g,K_b)=(0.2126,0.7152,0.0722)$ を用いた
フルレンジの簡易変換を採用する（オフセットや量子化レンジは扱わない）．
...
\subsection{sRGB の逆EOTF（linear light への復元）}
実写実験では入力 $x_{\mathrm{srgb}}\in[0,1]$ を linear light に統一するため，
成分ごとに sRGB 逆EOTF を適用する：
\[
E^{-1}(v)=
\begin{cases}
...
\noindent
\textit{注} これらの式は規格の完全な実装条件（符号化レンジ・量子化・オフセット等）を省略した
簡約形であり，本稿では順序不整合の効果を明確化する目的で用いる．
